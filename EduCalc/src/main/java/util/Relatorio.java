...
public class Relatorio {
    ...
    // Adiciona informações de um aluno ao relatório
    public void adcAluno(
        ...
        Aluno alunoProcurado = new Aluno(matriculaAluno);

        if (alunoProcurado.IndiceAluno(alunoProcurado, listaAlunos) != 0) {
            conteudo.append("Nome do Aluno: ")
                    .append(alunoProcurado.nomeAluno(matriculaAluno, listaAlunos))
                    .append("\n");

            // Percorre os cursos do aluno
            No<Curso> aux = listaCursos.getHead();
            No<Disciplina> nome = listaDisciplinas.getHead();

            while (aux != null) {
                if (aux.getConteudo().getMatricCurso() == alunoProcurado.getMatriculaAluno()) {
                    conteudo.append("Disciplina: ")
                            .append(nome.getConteudo().nomeDisciplina(aux.getConteudo().getCodDisciplina(), listaDisciplinas))
    ...
    // Adiciona informações de uma disciplina ao relatório
    public void adcDisciplina(
        ListaDuplamenteEncadeada<Aluno> listaAlunos,
        ListaDuplamenteEncadeada<Disciplina> listaDisciplinas,
        ListaDuplamenteEncadeada<Curso> listaCursos,
        int codDisciplina
    ) throws IOException {
        No<Disciplina> disc = listaDisciplinas.getHead();
        System.out.println(disc.getConteudo().getIndice(codDisciplina, listaDisciplinas));
        ...
        if (disc.getConteudo().getIndice(codDisciplina, listaDisciplinas) != 0) {
            ...
            No<Curso> noCursoAux = listaCursos.getHead();
            No<Aluno> noAlunoAux = listaAlunos.getHead();

            while (noCursoAux != null) {
                if (noCursoAux.getConteudo().getCodDisciplina() == codDisciplina) {
                    conteudo.append("Aluno: ")
                            .append(noAlunoAux.getConteudo().nomeAluno(noCursoAux.getConteudo().getMatricCurso(), listaAlunos))
                            .append("\n");
                }
                noCursoAux = noCursoAux.getProximo();
            }
